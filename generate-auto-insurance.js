...
const states = [
  { name: 'California', abbr: 'CA', avgPremium: 2190, minCoverage: '15/30/5' },
  { name: 'Texas', abbr: 'TX', avgPremium: 1850, minCoverage: '30/60/25' },
  { name: 'Florida', abbr: 'FL', avgPremium: 2560, minCoverage: '10/20/10' },
  { name: 'New York', abbr: 'NY', avgPremium: 2750, minCoverage: '25/50/10' },
  { name: 'Pennsylvania', abbr: 'PA', avgPremium: 1680, minCoverage: '15/30/5' },
  { name: 'Illinois', abbr: 'IL', avgPremium: 1520, minCoverage: '25/50/20' },
  { name: 'Ohio', abbr: 'OH', avgPremium: 1250, minCoverage: '25/50/25' },
  { name: 'Georgia', abbr: 'GA', avgPremium: 1890, minCoverage: '25/50/25' },
  { name: 'Michigan', abbr: 'MI', avgPremium: 2680, minCoverage: '50/100/10' },
  { name: 'New Jersey', abbr: 'NJ', avgPremium: 2100, minCoverage: '15/30/5' }
];
...
<head>
    ...
    <title>${state.name} Auto Insurance Calculator 2026 - Free Car Insurance Estimate</title>
    <meta name="description" content="Calculate car insurance costs in ${state.name}. Free ${state.abbr} auto insurance calculator with coverage options, discounts, and quotes.">
    <meta name="keywords" content="${state.name} auto insurance, ${state.abbr} car insurance calculator, ${state.name} car insurance cost, cheap car insurance ${state.abbr}">
...
<body>
    <div class="auto-container">
        <h1>🚗 ${state.name} Auto Insurance Calculator</h1>
        <p>Calculate car insurance costs in ${state.name} (${state.abbr}). Get estimated premiums based on coverage, vehicle, and driver profile.</p>
        ...
        <div class="stat-grid">
            ...
            <div class="stat-card">
                <div class="value">${state.minCoverage}</div>
                <div>Min. Coverage Required</div>
            </div>
            <div class="stat-card">
                <div class="value">$${Math.round(state.avgPremium/12).toLocaleString()}</div>
                <div>Avg. Monthly Payment</div>
            </div>
        </div>

        <div class="calc-card">
            <h2>Calculate Your ${state.abbr} Auto Insurance Cost</h2>
            ...
            <div class="input-group">
                ...
                <select id="coverage">
                    <option value="minimum">Minimum Required (${state.minCoverage})</option>
            ...
            <div id="results" class="result-box">
                <h3>Your Estimated ${state.abbr} Auto Insurance Cost</h3>
        ...
        <div class="info-box">
            <h3>${state.name} Auto Insurance Requirements</h3>
            <p><strong>Minimum Coverage:</strong> ${state.minCoverage} (Bodily Injury per person / per accident / Property Damage)</p>
            <p><strong>Average Premium:</strong> $${state.avgPremium.toLocaleString()}/year ($${Math.round(state.avgPremium/12)}/month)</p>
            <p><strong>Penalties:</strong> Driving without insurance in ${state.name} can result in fines up to $1,000, license suspension, and vehicle impoundment.</p>
        </div>

        <h2>How to Lower Your ${state.name} Car Insurance</h2>
        <ul>
  ...
  fs.writeFileSync(`auto/${slug}-auto-insurance.html`, html);
  console.log(`✓ Created auto/${slug}-auto-insurance.html`);
});

console.log(`\n✅ Generated ${states.length} state auto insurance calculators`);
